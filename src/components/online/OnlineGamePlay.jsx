...
import {
    Eye, EyeOff, RotateCcw, Vote, Mic, User
} from "lucide-react";
...
const OnlineGamePlay = ({ onEnd }) => {
    ...
    const { socket, room, onlinePhase, onlineGameData, votedPlayers, syncPhase, submitVote } = useSocket();
                    ...
                    >
                        ...
                        <div className="absolute inset-0 backface-hidden rotate-y-180 game-card glass-panel flex flex-col items-center justify-center border-4 border-primary shadow-2xl overflow-hidden">
                            <motion.div animate={isRevealed ? revealCard : {}} className="text-center p-6 w-full">
                                ...
                                <h3 className={`text-2xl font-black mb-4 uppercase ${isImpostor ? 'text-destructive' : 'text-primary'}`}>
                                    {isImpostor ? t("game.youAreImpostor") : onlineGameData?.secretWord}
                                </h3>
                                {!isImpostor && (
                                    <p className="text-muted-foreground font-bold">
                                        {t("game.categoryHint")}: {t(onlineGameData?.category)}
                                    </p>
                                )}
                            </motion.div>
                        </div>
    ...
    if (onlinePhase === "result") {
        return (
            <div className="min-h-screen bg-background flex flex-col items-center justify-center px-5 relative overflow-hidden text-center">
                <h2 className="text-5xl font-black mb-8 text-primary uppercase">{t("game.result")}</h2>
                <div className="game-card max-w-sm w-full py-12">
                    <RotateCcw className="w-16 h-16 text-primary mx-auto mb-6" />
                    <button onClick={() => window.location.reload()} className="glass-button px-8 py-4 rounded-2xl font-bold text-xl">
                        {t("game.newRound")}
                    </button>
                </div>
            </div>
